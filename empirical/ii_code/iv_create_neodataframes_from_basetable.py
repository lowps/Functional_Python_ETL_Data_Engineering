...
def create_exited_salary_correlation(df: pd.DataFrame) ->pd.DataFrame:
    df_salary= df[['geography','gender','exited','estimatedsalary']].groupby(['geography','gender']).agg({'estimatedsalary':'mean'}).sort_values(by='estimatedsalary', ascending=True)
